...
- Token：macOS `~/Library/Application Support/kiro-cli/data.sqlite3`；Windows `%APPDATA%\kiro-cli\data.sqlite3`（兼查 `%LOCALAPPDATA%`、`~\.kiro-cli`、`~\.local\share\kiro-cli`）；Linux `$XDG_DATA_HOME/kiro-cli` 或 `~/.kiro-cli`。表 `auth_kv`（key `kirocli:social:token`，旧版 `kirocli:odic:token`；JSON 含 access_token/expires_at/profile_arn）。CLI 负责刷新——过期就跑一次 kiro-cli。
